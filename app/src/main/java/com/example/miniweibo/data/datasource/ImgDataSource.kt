...
class ImgDataSource @Inject constructor(
    ...
    fun getImg(scope: CoroutineScope): LiveData<Resource<List<String>>> {
        return object : ScopeDataSource<List<String>, List<String>>(scope) {
            override suspend fun loadData(): LiveData<ApiResponse<List<String>>> {
                return api.getPic()
            }

            override fun loadFromDb(): LiveData<List<String>> = db.imgDao().selectAll()
